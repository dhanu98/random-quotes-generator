var quotes = [
	...
	{
	quote: "The good things don't always soften the bad things, but vice versa, the bad things don't always spoil the good things and make them unimportant.",
	source: "Doctor Who"
	},
	{
...
function generateQuote(){
	var ran = Math.floor(Math.random()*37);
  document.getElementById('quote').innerHTML = '"' + quotes[ran]['quote'] + '"'
  document.getElementById('source').innerHTML = '-' + quotes[ran]['source']
	document.body.style.background = getColor();
}

function result(event){
  document.getElementById('btn').addEventListener(event,generateQuote);
}
